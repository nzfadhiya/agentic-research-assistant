    ...
    with st.popover("+ Upload / Generate", use_container_width=True):
        ...
        with tab_gen:
                    ...
                    with st.spinner("Generating " + doc_format.upper() + "..."):
                        try:
                            ...
                            if r.status_code == 200:
                                ...
                                st.session_state.messages.append({"role": "assistant",
                                    "content": "Your **" + doc_format.upper() + "** document on *" + doc_topic + "* is ready. Use the 👁️ / ⬇️ / 🔄 / 🗑️ buttons in the sidebar — 👁️ shows a preview here in the chat, ⬇️ downloads it."})
                                refresh_sessions()
                            else:
                                st.error("Error: " + r.json().get("detail", "Failed"))
                        except Exception as e:
            ...
            if st.session_state.get("doc_ready") and st.session_state.get("doc_data"):
                ...
                with ic1:
                    if st.button("👁️", key="doc_preview_icon", help="Preview", use_container_width=True):
                ...
                with ic3:
                    if st.button("🔄", key="doc_regen_icon", help="Regenerate", use_container_width=True):
                        ...
                        st.rerun()
                with ic4:
                    if st.button("🗑️", key="doc_dismiss_icon", help="Dismiss", use_container_width=True):
                        st.session_state.doc_ready = False
                        st.session_state.doc_data = None
                        st.session_state.doc_preview_content = ""
                        st.session_state.doc_preview_text = ""
